...
$mod_strings = array (
    ...
    'LBL_FORECASTS_CONFIG_HELP_RANGES' => '对您首选的 {{forecastByModule}} 分类方式进行配置。<br><br>请注意，首次提交后无法更改范围设置。对于升级后的实例，范围设置使用现有预测数据进行锁定。<br><br>您可以根据概率范围选择两个或多个类别，或者不根据概率创建类别。<br><br>自定义类别的左侧有复选框；请使用这些复选框确定向经理提交和报告的预测金额中将要包含的范围。<br><br>用户可以从其工作单手动更改 {{forecastByModule}} 的包含/排除状态和类别。',
